"""Database CRUD operations - 适配SQLite数据库结构."""
...
from app.database.models import (
    Contract, Employee, AttendanceRecord, ConversationRecord,
    SalaryRecord, EmployeeResume, InterviewRecord
)
...
class SalaryRecordCRUD:
    """CRUD operations for SalaryRecord model."""
...
class InterviewRecordCRUD:
    """CRUD operations for InterviewRecord model."""

    @staticmethod
    async def get_by_employee(
        db: AsyncSession, employee_id: int
    ) -> List[InterviewRecord]:
        """Get interview records by employee ID."""
        result = await db.execute(
            select(InterviewRecord).where(InterviewRecord.employee_id == employee_id)
        )
        return list(result.scalars().all())


class EmployeeResumeCRUD:
    """CRUD operations for EmployeeResume model."""

    @staticmethod
    async def get_by_employee(
        db: AsyncSession, employee_id: int
    ) -> Optional[EmployeeResume]:
