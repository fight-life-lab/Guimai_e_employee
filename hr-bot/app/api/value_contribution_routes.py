...
class ValueContributionImportRequest(BaseModel):
    """价值贡献导入请求（Base64编码）"""
    filename: str
            ...
            try:
                ...
                if 'performance_standard' in df.columns and pd.notna(row['performance_standard']):
                    ...
                    if isinstance(performance_standard, str):
                        ...
                        performance_standard = float(performance_standard)
                ...
                if 'deviation_rate' in df.columns and pd.notna(row['deviation_rate']):
                    ...
                    # 处理偏离度值：
                    # - 如果 0 < value < 1（如 0.98），表示 98%，需要乘以 100
                    # - 如果 1 <= value < 10（如 1.14），表示倍数，需要乘以 100 转为百分比
                    # - 如果 value >= 10（如 98.6），已经是百分比，保持不变
                    if 0 < deviation_rate < 1:
                        deviation_rate = deviation_rate * 100
                    elif 1 <= deviation_rate < 10:
                        # 1.146552 这种形式，乘以 100 得到 114.66%
                        deviation_rate = deviation_rate * 100
                ...
                # 获取分数调整值（加分/扣分）
                score_adjustment = 0
                if 'score_adjustment' in df.columns and pd.notna(row['score_adjustment']):
                    score_adjustment = float(row['score_adjustment'])

                # 计算基础分数
                if deviation_rate is not None:
                    base_score = calculate_value_contribution_score(deviation_rate)
                else:
                    base_score = 70  # 默认基础分

                # 最终分数 = 基础分 + 调整分
                score = base_score + score_adjustment
...
@router.post("/import-base64", response_model=ValueContributionScoreResponse)
async def import_value_contribution_scores_base64(
    request: ValueContributionImportRequest,
    db: Session = Depends(get_db_session)
):
    """
    批量导入价值贡献分数（Base64编码，用于绕过WAF限制）
    ...
    try:
        ...
        # 检查文件类型
        if not request.filename.endswith(('.xlsx', '.xls', '.csv')):
            return ValueContributionScoreResponse(
                success=False,
                message="只支持Excel文件格式(.xlsx, .xls)和CSV文件(.csv)",
                errors=["文件格式错误"]
        ...
        for index, row in df.iterrows():
            try:
                ...
                # 获取考核年度（优先从Excel读取，否则使用传入的参数或当前年份）
                row_evaluation_year = request.evaluation_year
                if 'evaluation_year' in df.columns and pd.notna(row['evaluation_year']):
                    row_evaluation_year = int(row['evaluation_year'])

                if not row_evaluation_year:
                    row_evaluation_year = datetime.now().year
                ...
                if existing:
                    ...
                    existing.actual_performance = actual_performance
                    existing.deviation_rate = deviation_rate
                    existing.evaluator = request.evaluator
                    existing.evaluation_basis = f"批量导入于{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                    existing.remark = remark
        ...
        db.rollback()
        logger.error(f"批量导入价值贡献分数(Base64)失败: {e}")
        return ValueContributionScoreResponse(
            success=False,
            message=f"导入失败: {str(e)}",
            errors=[str(e)]
