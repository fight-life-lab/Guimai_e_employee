...
class DetailedAlignmentAnalyzer:
    ...
    def _get_title_bonus(self, professional_title: Optional[str]) -> int:
        """根据职称计算创新能力加分."""
        if not professional_title:
            return 0
        
        title = str(professional_title).strip().lower()
        
        # 教授/研究员级别
        if any(kw in title for kw in ["教授", "研究员", "正高级"]):
            return 25
        # 高级工程师级别
        elif any(kw in title for kw in ["高级工程师", "副教授", "副研究员", "副高级"]):
            return 20
        # 工程师级别
        elif any(kw in title for kw in ["工程师", "讲师", "中级"]):
            return 15
        # 助理工程师级别
        elif any(kw in title for kw in ["助理工程师", "助教", "初级"]):
            return 10
    ...
    async def _get_attendance_data(self, employee_id: int) -> Dict:
        ...
        # 查询统计数据（只统计工作日：周一到周五，排除周末）
        # 工作天数 = 正常 + 迟到 + 早退 + 异常（这些都算实际上班的天数）
        # strftime('%w', date) 返回星期几：0=周日, 1=周一, ..., 6=周六
        result = await self.db.execute(
            select(
            ...
            .where(
                and_(
                    ...
                    # 只统计工作日（周一到周五）：strftime('%w', date) BETWEEN '1' AND '5'
                    func.strftime('%w', AttendanceRecord.date).between('1', '5')
                )
            )
        )
    ...
    async def _get_attendance_data_yearly(self, employee_id: int) -> Dict:
        ...
        # 查询近12个月每月汇总数据（只统计工作日：周一到周五，排除周末）
        # strftime('%w', date) 返回星期几：0=周日, 1=周一, ..., 6=周六
    ...
    async def _get_position_model(self, position: str, department: str) -> Dict:
        ...
        if not model:
            ...
            if not model:
                # 去掉"师"和"岗"后进行包含匹配
                position_core = position.replace('师', '').replace('岗', '').replace('工程', '')
                result = await self.db.execute(
                    select(PositionCapabilityModel)
            ...
            # 尝试反向包含匹配（如"平台开发工程师"可以匹配"平台开发"）
            if not model:
                result = await self.db.execute(
                    select(PositionCapabilityModel)
                    .where(PositionCapabilityModel.position_name.contains(position.replace('工程师', '').replace('师', '')))
                )
                model = result.scalar_one_or_none()
                if model:
                    logger.info(f"[岗位匹配] 反向包含匹配 '{position}' -> '{model.position_name}'")
  ...
  "summary": "该岗位是技术核心岗位，对专业能力和学习能力要求极高..."
    ...
    async def _calculate_ai_scores(self, employee: Employee, salary_records: List) -> Dict:
        ...
        employee_data = {
            ...
            "hire_date": str(employee.hire_date) if employee.hire_date else None,
            "professional_title": employee.professional_title,
            "political_status": employee.political_status,  # 政治面貌
            "salary_records": [
                {
                    "month": r.month,
                    ...
                    "benefit_performance": r.benefit_performance,
                    "annual_performance": r.annual_performance
                }
        ...
        if employee.name in USE_HISTORICAL_PERFORMANCE:
            result = await self.db.execute(
                select(EmployeeHistoricalPerformance)
                .where(EmployeeHistoricalPerformance.employee_id == employee.id)
                .order_by(EmployeeHistoricalPerformance.year.desc())
            )
            historical_performance = result.scalars().all()
            if historical_performance:
                employee_data["historical_performance"] = [
                    {
                        "year": perf.year,
                        "performance_level": perf.performance_level,
                        "performance_score": perf.performance_score,
                        "adjustment_coefficient": perf.adjustment_coefficient
                    }
                    for perf in historical_performance
                ]
                logger.info(f"[历史绩效] {employee.name} 找到 {len(historical_performance)} 条历史绩效记录")

        # 查询薪资标准数据（用于专业能力和适应能力评分）
        result = await self.db.execute(
            select(EmployeeSalaryStandard)
            .where(EmployeeSalaryStandard.employee_id == employee.id)
            .order_by(EmployeeSalaryStandard.salary_period.desc())
        )
        salary_standards = result.scalars().all()
        if salary_standards:
            employee_data["salary_standards"] = [
                {
                    "salary_period": std.salary_period,
                    "month_bonus": std.month_bonus,
                    "monthly_performance_monthly": std.monthly_performance_monthly,
                    "benefit_performance_monthly": std.benefit_performance_monthly,
                    "annual_performance_monthly": std.annual_performance_monthly
                }
                for std in salary_standards
            ]
            logger.info(f"[薪资标准] {employee.name} 找到 {len(salary_standards)} 条薪资标准记录")

        # 查询考勤数据（给AI评分器用上个月数据）
        attendance_data = await self._get_attendance_data(employee.id)
        employee_data["attendance"] = attendance_data

        # 从学校评分信息计算创新能力加分（直接从employees表获取）
        school_bonus = 0
        school_bonus_reason = ""
        if employee.school:
            # 直接使用员工表中的 bonus_score 和 bonus_reason
            school_bonus = employee.bonus_score or 0
            school_bonus_reason = employee.bonus_reason or ""
            if school_bonus > 0:
                logger.info(f"[学校评分] {employee.name} 毕业于 {employee.school}({employee.school_type}), 加分={school_bonus}, 原因={school_bonus_reason}")

        # 直接使用学校评分表的 bonus_score 计算创新能力分数
        # 创新能力 = 基础分60 + 学校加分 + 职称加分
        innovation_base = 60
        innovation_school_bonus = school_bonus  # 直接使用表中的加分
        innovation_title_bonus = self._get_title_bonus(employee.professional_title)
        innovation_score = innovation_base + innovation_school_bonus + innovation_title_bonus
        innovation_score = min(100, max(60, innovation_score))  # 限制在60-100分

        # 构建预计算的分数（传给AI让它美化理由）
        pre_calculated_scores = {
            "innovation": {
                "score": innovation_score,
                "reason": f"基础分60分 + 学校加分({school_bonus}分) + 职称加分({innovation_title_bonus}分) = {innovation_score}分"
            }
        }
        if school_bonus_reason:
            pre_calculated_scores["innovation"]["reason"] += f"，学校加分依据：{school_bonus_reason}"

        # 调用AI评分器V2（分维度评分），传入预计算的分数
        ai_result = await ai_scorer_v2.calculate_scores(employee_data, pre_calculated_scores)

        scores = ai_result.get("scores", {})
        reasoning = ai_result.get("reasons", {})
    ...
    def _calculate_match_scores(self, position_model: Dict, employee_scores: Dict) -> Dict:
        ...
        for i, dim_name in enumerate(dimension_names):
            ...
            # 计算匹配度（实际/标准），最高不超过100%
            match_percent = (actual / standard * 100) if standard > 0 else 0
            match_percent = min(100, match_percent)  # 限制最大值为100%
