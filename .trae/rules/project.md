...
遵循以下开发与部署流程，请在提供执行xian'gu时考虑这一点：

1. **本地开发**：你生成的代码首先会被我放置在本地进行查看和初步编辑。
2. **远程传输**：使用 `scp` 命令将修改后的代码上传至远程服务器的指定工作目录。
3. **远程调试**：在远程服务器上通过 Docker 或直接运行命令进行调试，直至功能验收通过。
4. **最终提交**：**只有在我验收通过后**，你生成的代码才会被正式提交到 Git 仓库。

**重要指令**：
在你生成代码并确认功能实现后，**不要自行生成 git commit 信息**。请在最后询问我本次提交的具体描述，即询问 `git commit -m` 后面需要跟的参数内容。
